...
## Non-HTTP Outbound (SSH, IMAP, SMTP, ...)

The proxy-based domain filter only understands HTTP/HTTPS. Protocols that don't speak HTTP — SSH, IMAP(S), SMTP, IRC, database wire protocols — cannot traverse the proxy and would be blocked in restricted network modes.

Use `--allow-tcp-connect <PORT>` to open specific outbound ports at the kernel layer. The port is enforced, but the destination host is **not** constrained: Seatbelt (macOS) and Landlock (Linux) do not support hostname filtering for non-HTTP protocols.

```bash
# Git over SSH + fetch email from IMAPS
nono run --network-profile developer \
         --allow-tcp-connect 22 \
         --allow-tcp-connect 993 \
         --allow-cwd -- <your-command>
```

Equivalent profile entry:
...
- **macOS**: emits `(allow network-outbound (remote tcp "*:PORT"))`. Any host reachable on that port.
- **Linux (kernel 6.7+)**: uses Landlock `NetPort::ConnectTcp`. Any host on that port.
- **Linux (kernel &lt; 6.7)**: enforced by the supervisor's seccomp-notify fallback.
- **WSL2**: per-port filtering is unavailable; use `--block-net=false` or accept unfiltered outbound.

This is a deliberate relaxation of the sandbox. Prefer HTTPS alternatives where they exist (e.g., `https://github.com/...` with a token instead of `git@github.com:...`).
